...
class GpModeling:
    def __init__(self, measurement_noise, optimizer='fmin_l_bfgs_b', n_restarts_optimizer=0, area_largest_length=50, active_set_size=100):
        self.is_trained = False
        self.active_set_size = active_set_size
        length_scale_bounds=(1e-05, 50*10)
    ...
    def update(self, X, y):
        ...
        if n >= n_max:
            obs = downsample(np.hstack((X, y.reshape(y.shape[0], 1))))
            Xe = obs[:, :2]
            ye = obs[:, 2]
        else:
            Xe = X

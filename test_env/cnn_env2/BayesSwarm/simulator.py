...
class Simulator:
    def __init__(self, n_robots, source_id, start_locations=None, decision_making_mode="bayes-swarm",
            ...
            depot_mode="single-depot", enable_log_simulation=True, simulation_configs=None, mission=0):
        ...
        # self.decision_time = np.array([0, 0, 0])
        ...
        for robot_id in range(n_robots):
            ...
            if self.is_scout_team and robot_id == 0:
                ...
                self.robots[robot_name] = Robot(robot_id, n_robots, self.bayes_swarm_args, signal_source,
                                                velocity, start_location, source_detection_range,
                                                observation_frequency, measurement_noise_rate, self.mission)
    ...
    def run(self):
        ...
        self.mission_time = np.round(t, 5)
        _, decision_counter, decision_computing_time = self.robots[robot_successed].get_data()
        self.decision_time = np.array(decision_computing_time).reshape(1, len(decision_computing_time))[0]
    ...
    def plot_robot_trajectory(self):
        ...
        for irobot in self.robots:
            ...
            X_obs = robots_observations[:, :2]
            y_obs = robots_observations[:, 2]
            obs = np.hstack((X_obs, y_obs.reshape(len(y_obs), 1)))

            # print("Robot-", irobot, " : ", len(robots_observations))
            # ax.plot(robots_trajectory[:,0], robots_trajectory[:,1], '-', color="#b9e9e9", label=irobot)
            ...
            if self.is_plot_per_decision:
                if self.reached_robot_id == irobot:
                    if not len(expected_source_location) == 0:
                        ax.plot(expected_source_location[0], expected_source_location[1], 'X', color=color, label=irobot+"-expected-source")
                    # ax.plot(robots_trajectory[:,0], robots_trajectory[:,1], '-', linewidth=2, color=color, label=irobot)
                    ax.plot(waypoints[:,0], waypoints[:,1], '-s', markersize=5, linewidth=2, color=color, label=irobot+"-w")
                    if len(y_obs) > 110:
                        down_obs = downsample(obs)
                        ax.scatter(down_obs[:,0], down_obs[:,1], color=color, label=irobot+"-w")

                    if self.decision_making_mode == "bayes-swarm":
                        Y_pred, _ = self.robots[irobot].gp_contour_data("belief", X1_true, X2_true)                                
                        mapping_error = np.mean(np.abs(Y_true - Y_pred))
                        print("Robot-", irobot, " : ", mapping_error)
        ...
        counter = self.robots[self.reached_robot_id].decision_counter
        #ax.legend()
        if self.is_save_fig:
            plt.savefig(self.file_name+"/" + str(self.current_time) + f"_{self.reached_robot_id}_decision_{counter}_counter" + str(self.fig_counter)+'.pdf'

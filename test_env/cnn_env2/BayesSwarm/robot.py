...
from BayesSwarm.filtering import Filtering, downsample


class Robot:
    def __init__(self, id, n_robots, bayes_swarm_args, source,
                 velocity=1, start_location=[0,0], source_detection_range=1e-1,
                 observation_frequency=1, measurement_noise_rate=0, mission=0):
    ...
    def plot_contour_data(self, robot_path):

        X1_true, X2_true, Y_true = self.source.get_data_for_plot()
        N = 25
        X = np.hstack((X1_true.reshape(-1, 1), X2_true.reshape(-1, 1)))
        ymean, ystd = self.gp_contour_data('', X1_true, X2_true)
        plt.rcParams['font.family'] = 'Arial'

        fig, ax = plt.subplots(figsize=(4, 4))
        ax.contour(X1_true, X2_true, Y_true, colors='gray')
        ax.contour(X1_true, X2_true, ymean, linestyles='dashed')
        ax.plot(self.waypoint_history[:,0], self.waypoint_history[:,1], '--s', markersize=3, color='orange')
        plt.tight_layout()
        fig.savefig(robot_path + f'/decision_{self.decision_counter}.png', format='png', dpi=300, pad_inches=0)

        fig2, ax2 = plt.subplots(figsize=(4, 4))
        ax2.contourf(X1_true, X2_true, ystd, linestyles='dashed')
        fig2.savefig(robot_path + f'/uncertainty_{self.decision_counter}.png', format='png', dpi=300, pad_inches=0)
    ...
    def plan_next_waypoint(self, t):
        ...
        # Uncomment the following lines to get the evolving belief model of robots per waypoint
        # robot_path = os.path.join(f'output_{self.mission}', f'robot_{self.id}')
        # if not os.path.exists(robot_path):
        #     os.makedirs(robot_path)

        # if self.decision_counter > 0:
        #     self.plot_contour_data(robot_path)

        #     if np.size(y_robot) > 100:
        #         fig3, ax3 = plt.subplots(figsize=(4, 4))
        #         ax3.scatter(X_robot[:, 0], X_robot[:, 1])
        #         fig3.savefig(robot_path + f'/complete_{self.decision_counter}.png', format='png', dpi=300, pad_inches=0)

        #         obs = downsample(np.hstack((X_robot, y_robot.reshape(y_robot.shape[0], 1))))
        #         Xe = obs[:, :2]
        #         fig4, ax4 = plt.subplots(figsize=(4, 4))
        #         ax4.scatter(Xe[:, 0], Xe[:, 1])
        #         fig4.savefig(robot_path + f'/cnn_{self.decision_counter}.png', format='png', dpi=300, pad_inches=0)

            # plt.show()

        # if self.decision_counter > 0 and self.id == 18:
            # os.system('pause')
        self.waypoint_start = self.location

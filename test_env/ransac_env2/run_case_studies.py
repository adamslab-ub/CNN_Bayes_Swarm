...
def main():
    debug = False
    case_studies = [820, 835, 850, 420, 435, 450]
    for cs in case_studies:
        ...
        if not os.path.exists(f"ransac/{case_study}"):
            os.makedirs(f"ransac/{case_study}")

        # if not os.path.exists(f"ransac/{case_study}/decision_time"):
        #     os.makedirs(f"ransac/{case_study}/decision_time")

        # if not os.path.exists(f"ransac/{case_study}/decision_time_mission"):
        #     os.makedirs(f"ransac/{case_study}/decision_time_mission")
        ...
        for mission in range(10):
            ...
            with open(f"ransac/{case_study}/decision_time_{mission}.csv", 'w', newline='') as file:
                writer = csv.writer(file)
                for tm in decision_time:
                    writer.writerow([tm])

            with open(f"ransac/mission_time_{case_study}.csv", 'w', newline='') as file:
                writer = csv.writer(file)
                for tm in arr_req:
                    writer.writerow([tm])

            with open(f"ransac/decision_time_{case_study}.csv", 'a', newline='') as file:
                writer = csv.writer(file)
